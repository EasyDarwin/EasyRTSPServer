...
public:
  ...
  virtual Boolean SourceAddrOKForRelaying(UsageEnvironment& env,
	  LIVE_NET_ADDRESS_INADDR addr) = 0;		//gavin 2018.03.19
					  //unsigned addr) = 0;
...
public:
  ...
  virtual Boolean handleRead(unsigned char* buffer, unsigned bufferMaxSize,
			     unsigned& bytesRead,
			     LIVE_NET_ADDRESS_SOCKADDR_IN& fromAddress) = 0;
      // Returns False on error; resultData == NULL if data ignored

  int socketNum() const { return fSocketNum; }
  LIVE_IP_VER_ENUM socketIPVer()	const {return fSocketIPVer;}
  void SetSocketIPVer(LIVE_IP_VER_ENUM ipVer)	{fSocketIPVer = ipVer;}
...
protected:
  Socket(UsageEnvironment& env, Port port, int clientSock, LIVE_IP_VER_ENUM ipVer); // virtual base class

  Boolean changePort(Port newPort); // will also cause socketNum() to change

private:
  int fSocketNum;
  LIVE_IP_VER_ENUM	fSocketIPVer;
