...
public:
  ...
  int	GetEnvirId()	{return envirId;}
  char *GetEnvirName()	{return envirName;}

  void	SetEnvirCount(int count)	{envirCounter = count;}
  void	IncEnvirCount()				{envirCounter ++;}
  int	GetEnvirCount()				{return envirCounter;}
  void	ResetInternalChannel(int val, void *pServerMediaSession)		{
		resetInternalChannel = val; 
		if (val!=0x05)
		{
			mTmpServerMediaSession=pServerMediaSession;
		}
  }
  int	IsResetInternalChannel()			{return resetInternalChannel;}
  void	*GetTmpServerMediaSession() {return mTmpServerMediaSession;}
  void  IncrementReferenceCount()	{clientReferenceCount ++;}
  void	DecrementReferenceCount()	{if (clientReferenceCount>0)	clientReferenceCount --;}
  int	GetReferenceCount()			{return clientReferenceCount;}
  void	SetStreamStatus(int _status);//	{streamStatus = _status;}
  int	GetStreamStatus()	;//		{return streamStatus;}
  void	SetStreamName(const char *streamName)			{
	  memset(liveStreamName, 0x00, sizeof(liveStreamName));
	  if (NULL == streamName)
	  {
		  return;
	  }
	  strcpy(liveStreamName, streamName);
  }
  char *GetStreamName()		{return liveStreamName;}

  void	SetLockFlag(int _flag)		{mLockFlag = _flag;}
  int	GetLockFlag()				{return mLockFlag;}

  void	LockEnvir(char *functionName, unsigned long long ptr);
  void	UnlockEnvir(char *functionName, unsigned long long ptr);

protected:
  UsageEnvironment(TaskScheduler& scheduler, int _envirId, char *_envirName); // abstract base class
  virtual ~UsageEnvironment(); // we are deleted only by reclaim()

  int	envirId;			//add by gavin
  char	envirName[36];		//add by gavin
  int	envirCounter;		//add by gavin
  int	resetInternalChannel;		//add by gavin
  void	*mTmpServerMediaSession;//add by gavin
  int	mLockFlag;			//add by gavin
  int	clientReferenceCount;//add by gavin
  int	streamStatus;			//add by gavin
  OSMutex	osMutex;			//add by gavin
  char	liveStreamName[512];
private:
...
public:
  ...
  virtual int setBackgroundHandling(int socketNum, int conditionSet, BackgroundHandlerProc* handlerProc, void* clientData) = 0;
  ...
  int turnOnBackgroundReadHandling(int socketNum, BackgroundHandlerProc* handlerProc, void* clientData) {
    return setBackgroundHandling(socketNum, SOCKET_READABLE, handlerProc, clientData);
  }
