...
void	BasicTaskScheduler0::ClearDelayQueue()
{
	fDelayQueue.ClearAll();
}
...
#ifdef _DEBUG1
	_TRACE(TRACE_LOG_DEBUG, (char*)"创建Handler   socketNum[%d]\n", socketNum);
#endif
  }
...
void HandlerSet::clearHandler(int socketNum) {
	...
	//_TRACE(TRACE_LOG_DEBUG, (char*)"========根据socketNum[%d] 删除handler.\n", socketNum);

  HandlerDescriptor* handler = lookupHandler(socketNum);
  delete handler;

#ifdef _DEBUG1		//gavin
  handler = lookupHandler(socketNum);
  if (NULL != handler)
  {
	  _TRACE(TRACE_LOG_DEBUG, (char*)"handler 不为空.\n");
  }
#endif
}
