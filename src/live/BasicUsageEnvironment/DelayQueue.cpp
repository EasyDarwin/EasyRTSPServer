...
DelayQueueEntry::~DelayQueueEntry() {
	//_TRACE(TRACE_LOG_DEBUG, (char*)"DelayQueueEntry::~DelayQueueEntry()\n");
}

void DelayQueueEntry::handleTimeout() {
	//_TRACE(TRACE_LOG_DEBUG, (char*)"DelayQueueEntry::handleTimeout().  delete.........\n");
  delete this;
}
...
DelayQueue::DelayQueue()
  : DelayQueueEntry(ETERNITY) {
	  ...
	  //_TRACE(TRACE_LOG_DEBUG, (char*)"DelayQueue::DelayQueue()\n");
...
void DelayQueue::addEntry(DelayQueueEntry* newEntry) {
  ...
  if (NULL == cur)		return;		//add by gavin 2018.03.21
  while (cur && (newEntry->fDeltaTimeRemaining >= cur->fDeltaTimeRemaining)) {
    newEntry->fDeltaTimeRemaining -= cur->fDeltaTimeRemaining;
    cur = cur->fNext;
  }

  if (NULL != cur)		//add by gavin 2018.03.22
  {
...
void DelayQueue::removeEntry(DelayQueueEntry* entry) {
  ...
  //_TRACE(TRACE_LOG_DEBUG, (char*)"DelayQueue::removeEntry.....\n");
}
...
DelayInterval const& DelayQueue::timeToNextAlarm() {
	//add by gavin 2018.03.21
	DelayQueueEntry *pHead = head();
	if (NULL == pHead)		return DELAY_ZERO;	
...
void DelayQueue::handleAlarm() {
  ...
  if (head()->fDeltaTimeRemaining == DELAY_ZERO) {
	...
	//_TRACE(TRACE_LOG_DEBUG, (char*)"DelayQueue::handleAlarm()  Remove DelayQueueEntry.....\n");
...
void DelayQueue::synchronize() {
  ...
  if (NULL == curEntry)		return;	//add by gavin 2018.03.13
