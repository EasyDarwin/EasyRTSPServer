...
void GenericMediaServer::addServerMediaSession(UsageEnvironment	*pEnv, ServerMediaSession* serverMediaSession) {
  ...
  pEnv->SetStreamName(serverMediaSession->streamName());
...
#endif
{
	...
	while (1)
	{
		...
		{
			if (pThreadTask->pSubEnv->IsResetInternalChannel() > 0x00)
			{
				//resetChannel为1: 上层应用主动调用ResetChannel
				//resetChannel为5:  异常情况置0x05
				if (pThreadTask->pSubEnv->IsResetInternalChannel() == 0x01 || pThreadTask->pSubEnv->IsResetInternalChannel() == 0x05)
				{
					ServerMediaSession* sms = (ServerMediaSession*)pThreadTask->pSubEnv->GetTmpServerMediaSession();
					char streamName[512] = {0};

					if (pThreadTask->pSubEnv->IsResetInternalChannel() == 0x05)
					{
					...
					if (NULL != sms && sms->GetFlag() == LIVE_FLAG)
					{
						...
						pThreadTask->pSubEnv->ResetInternalChannel(0x02, NULL);
					...
					else if (NULL == sms)			//2019.12.30
					{
						strcpy(streamName, pThreadTask->pSubEnv->GetStreamName());

						//pThis->removeServerMediaSession(pThreadTask->pSubEnv, streamName, False);
						pThis->closeAllClientSessionsForServerMediaSession(streamName, True);
						pThis->CloseAllConnections(streamName);
					}

					pThreadTask->pSubEnv->ResetInternalChannel(0x00, NULL);
					//pTaskChedulerEpoll->ClearDelayQueue();
				}
				else if (pThreadTask->pSubEnv->IsResetInternalChannel() == 10)
				{
					ServerMediaSession* sms = (ServerMediaSession*)pThreadTask->pSubEnv->GetTmpServerMediaSession();
					//char streamName[512] = {0};
					//if (NULL != sms)
			...
			if (pThreadTask->releaseChannel > 0)
			{
				...
				if (pThreadTask->pSubEnv->GetReferenceCount() < 1)
				{
					...
					pThreadTask->pSubEnv->SetStreamStatus(0);
		...
		if (NULL != pThreadTask->pSubEnv && (pTaskScheduler0 = (BasicTaskScheduler0 *)&pThreadTask->pSubEnv->taskScheduler()))
		{
			if (pThreadTask->pSubEnv->IsResetInternalChannel() > 0x00)
			{
				pThreadTask->pSubEnv->LockEnvir("pThreadTask->pSubEnv->IsResetInternalChannel() > 0x00", (unsigned long long)pThreadTask);
				...
				pThreadTask->pSubEnv->UnlockEnvir("pThreadTask->pSubEnv->IsResetInternalChannel() > 0x00", (unsigned long long)pThreadTask);
	...
	int exitFlag = pThread->flag;

	pThread->flag = 0x00;

	if (exitFlag != 0x03)
	{
		DeleteOSThread(&pThreadTask->osThread);
	}
	...
	while ((connection = (GenericMediaServer::ClientConnection*)(iter->next(key))) != NULL) 
	{
		if (clientSocket == connection->fOurSocket)
		{
			if (connection->pClientConnectionEnv)
			{
				...
				connection->pClientConnectionEnv->ResetInternalChannel(0x05, NULL);
				/*
				//2019.12.30
		...
		for (int i=0; i<multiThreadCore.threadNum; i++)
		{
			...
			if ( 0 == strcmp(urlSuffix, multiThreadCore.threadTask[i].liveURLSuffix))
			{
				...
				if (assignEnv)
				{
					...
					//2019.12.30 如果工作线程已退出, 则重新创建工作线程
					//if (NULL == multiThreadCore.threadTask[i].osThread)
					{
						//CreateOSThread( &multiThreadCore.threadTask[i].osThread, __WorkerThread_Proc, (void *)&multiThreadCore.threadTask[i] );
					}
...
void		GenericMediaServer::PrintAllClientSessions()		//gavin 2020.05.11
{
  HashTable::Iterator* iter = HashTable::Iterator::create(*fClientSessions);
  GenericMediaServer::ClientSession* clientSession;
  char const* key; // dummy
