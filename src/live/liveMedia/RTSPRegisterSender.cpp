...
void RTSPRegisterSender::grabConnection(int& sock, LIVE_NET_ADDRESS_SOCKADDR_IN& remoteAddress) {
  sock = grabSocket();

	if (live_ip_ver == LIVE_IP_VER_4)
	{
		MAKE_SOCKADDR_IN(remoteAddr, fServerAddress.sin_addr.s_addr, htons(fRemoteClientPortNum));
	  memcpy(&remoteAddress.saddr4, &remoteAddr, sizeof(remoteAddr));
	}
	else
	{
		//gavin 2018.03.15  ??????
		MAKE_SOCKADDR_IN6(remoteAddr, fServerAddress.sin6_addr, htons(fRemoteClientPortNum));
	  memcpy(&remoteAddress.saddr6, &remoteAddr, sizeof(remoteAddr));
	}
}
