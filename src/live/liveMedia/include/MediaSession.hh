...
public:
  ...
  LIVE_NET_ADDRESS_INADDR const& sourceFilterAddr() const { return fSourceFilterAddr; }
...
protected:
  ...
  char* fAbsStartTime;
  char* fAbsEndTime;
  LIVE_NET_ADDRESS_INADDR fSourceFilterAddr; // used for SSM
...
public:
  ...
  char const* controlPath() const { return fControlPath; }
  Boolean isSSM(LIVE_IP_VER_ENUM ipVer) const { 
	  if (ipVer == LIVE_IP_VER_4)	  return fSourceFilterAddr.sin_addr.s_addr != 0; 
	  else
	  {
		  struct in6_addr sin6_addr;
		  memset(&sin6_addr, 0x00, sizeof(sin6_addr));

		  return memcmp(&sin6_addr, &fSourceFilterAddr.sin6_addr, sizeof(sin6_addr))==0 ? False : True;
	  }
  ...
  Boolean initiate(LIVE_IP_VER_ENUM ipVer, int useSpecialRTPoffset = -1);
  ...
  LIVE_NET_ADDRESS_INADDR connectionEndpointAddress() const;
      // Converts "fConnectionEndpointName" to an address (or 0 if unknown)
  void setDestinations(LIVE_NET_ADDRESS_INADDR defaultDestAddress);
