...
public:
  ...
  char* generateSDPDescription(int sock, LIVE_IP_VER_ENUM ipVer); // based on the entire session
      // Note: The caller is responsible for freeing the returned string

  char const* streamName() const { return fStreamName; }

  //int	GetStreamStatus()	{return fStreamStatus;}			//add by gavin
  //void	SetStreamStatus(int _status)	{fStreamStatus = _status;}		//add by gavin
...
public:
  ...
  char const* trackId();
  unsigned int GetFlag()		{return mFlag;}
  virtual char const* sdpLines(int clientSock, LIVE_IP_VER_ENUM ipVer) = 0;
  virtual void getStreamParameters(UsageEnvironment *pEnv, unsigned clientSessionId, // in
				   LIVE_NET_ADDRESS_INADDR clientAddress, // in
				   Port const& clientRTPPort, // in
				   Port const& clientRTCPPort, // in
				   int tcpSocketNum, // in (-1 means use UDP, not TCP)
				   unsigned char rtpChannelId, // in (used if TCP)
				   unsigned char rtcpChannelId, // in (used if TCP)
				   LIVE_NET_ADDRESS_INADDR& destinationAddress, // in out
  ...
  void setServerAddressAndPortForSDP(LIVE_NET_ADDRESS_INADDR addressBits,
...
protected: // we're a virtual base class
  ...
  LIVE_NET_ADDRESS_INADDR fServerAddressForSDP;
