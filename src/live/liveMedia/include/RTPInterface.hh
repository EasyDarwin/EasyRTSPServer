...
typedef struct __RTP_PACKET_T
{
	u_int8_t		*pbuf;		//1460*4;
	int				bufsize;
	int				bufpos;
...
#define	MAX_RTP_PACKET_NUM		2
  RTP_PACKE_T		fRTPPacket[MAX_RTP_PACKET_NUM];
  unsigned int		mFlag;
};
...
private:
  ...
  Boolean sendDataOverTCPEx(int socketNum, u_int8_t * data1, unsigned dataSize1, u_int8_t * data2, unsigned dataSize2, int level);
  Boolean sendRTPorRTCPPacketOverTCP(tcpStreamRecord* tcpStream, unsigned char* packet, unsigned packetSize);
private:
