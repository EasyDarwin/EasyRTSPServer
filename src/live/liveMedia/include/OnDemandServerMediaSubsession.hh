...
protected: // redefined virtual functions
  virtual char const* sdpLines(int clientSock, LIVE_IP_VER_ENUM ipVer);
  virtual void getStreamParameters(UsageEnvironment *pEnv, unsigned clientSessionId,
  ...
  virtual void startStream(UsageEnvironment& env, unsigned clientSessionId, void* streamToken,
  ...
  virtual void deleteStream(UsageEnvironment  *pEnv, unsigned clientSessionId, void*& streamToken);
...
protected: // new virtual functions, possibly redefined by subclasses
  ...
  virtual void	pauseStreamSource();			//gavin 2017.12.08
  virtual void  resumeStreamSource();			//gavin 2017.12.08
...
protected: // new virtual functions, may be redefined by a subclass:
  virtual Groupsock* createGroupsock(LIVE_NET_ADDRESS_INADDR const& addr, Port port, int clientSock, LIVE_IP_VER_ENUM ipVer);
...
private:
  void setSDPLinesFromRTPSink(RTPSink* rtpSink, FramedSource* inputSource,
			      unsigned estBitrate, LIVE_IP_VER_ENUM ipVer);
...
private:
  ...
  Boolean	fPauseStatus;
  int	tally;
  char	mStreamName[512];
  char	mMediaTypeName[16];
};
...
class Destinations {
public:
  Destinations(LIVE_NET_ADDRESS_INADDR const& destAddr, LIVE_IP_VER_ENUM _ipVer,
               Port const& rtpDestPort,
               Port const& rtcpDestPort)
    : ipVer(_ipVer), isTCP(False), addr(destAddr), rtpPort(rtpDestPort), rtcpPort(rtcpDestPort) {
  }
  Destinations(int tcpSockNum, LIVE_IP_VER_ENUM _ipVer, unsigned char rtpChanId, unsigned char rtcpChanId)
    : ipVer(_ipVer), isTCP(True), rtpPort(0) /*dummy*/, rtcpPort(0) /*dummy*/,
...
class StreamState {
public:
  StreamState(UsageEnvironment	*pEnv, OnDemandServerMediaSubsession& master,
  ...
  UsageEnvironment *GetEnvir()		{return fEnv;}

  void startPlaying(Destinations* destinations, UsageEnvironment& env, unsigned clientSessionId,
