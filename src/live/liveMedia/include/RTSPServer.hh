...
public:
  static RTSPServer* createNew(UsageEnvironment& env, Port ourPort = 554,
			       ...
			       unsigned reclamationSeconds = 65, void *_callback=NULL, void *_userptr=NULL);
  ...
  char* rtspURL(ServerMediaSession const* serverMediaSession, LIVE_IP_VER_ENUM ipVer, int clientSocket = -1) const;
  ...
  char* rtspURLPrefix(LIVE_IP_VER_ENUM ipVer, int clientSocket = -1) const;
...
protected:
  ...
  Boolean weImplementREGISTER(char const* cmd/*"REGISTER" or "DEREGISTER"*/,
  ...
  void implementCmd_REGISTER(char const* cmd/*"REGISTER" or "DEREGISTER"*/,
  ...
  virtual Boolean specialClientAccessCheck(int clientSocket, LIVE_NET_ADDRESS_SOCKADDR_IN& clientAddr,
  ...
  virtual Boolean specialClientUserAccessCheck(int clientSocket, LIVE_NET_ADDRESS_SOCKADDR_IN& clientAddr,
...
public: // should be protected, but some old compilers complain otherwise
  ...
  protected: // redefined virtual functions:
    virtual void handleRequestBytes(int newBytesRead, UsageEnvironment *pEnv);
  ...
  protected:
    ...
    virtual int  handleCmd_DESCRIBE(UsageEnvironment *pEnv, char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr, LIVE_THREAD_TASK_T **pThreadTask);
  ...
  protected:
    ...
    void closeSocketsRTSP(UsageEnvironment *pEnv);
	...
	int	clientRequestTrackNum;		//gavin 2019.02.21	rtp over tcp 时使用: server端sdp中有多个track, 而client请求时少于sdp中的track个数, 故需做计数
	char fClientSessionIdStr[16];	//gavin 2019.02.21  rtp over udp 时使用
  };
  ...
  protected:
    ...
    virtual int handleCmd_withinSession(RTSPClientConnection* ourClientConnection,
					 char const* cmdName,
	...
	//处理RTSP/RTCP
	virtual void handleCmd_RTCP(RTSPClientConnection* ourClientConnection, ServerMediaSubsession* subsession);
...
protected: // redefined virtual functions
  ...
  virtual ClientConnection* createNewClientConnection(int clientSocket, LIVE_NET_ADDRESS_SOCKADDR_IN clientAddr, LIVE_IP_VER_ENUM ipVer);

protected:
  // If you subclass "RTSPClientSession", then you must also redefine this virtual function in order
  // to create new objects of your subclass:
  virtual ClientSession* createNewClientSession(u_int32_t sessionId, UsageEnvironment	*pEnv);
...
private:
  ...
  void stopTCPStreamingOnSocket(UsageEnvironment *pEnv, int socketNum, int *clientTrackNum, char *clientSessionIdStr);//, Boolean assignSink);
...
private:
  ...
  OSMutex		osMutexTcpStreamingDatabase;

  void		LockTcpStreamingDatabase();
  void		UnlockTcpStreamingDatabase();

  public:

  //add by gavin
#define ENABLE_CLIENT_LIST		0x01		//0x01:启用记录客户端列表   0x00:不启用
#if ENABLE_CLIENT_LIST == 0x01
	  typedef struct __LIVE_CLIENT_INFO_T
	  {
		  RTSP_CLIENT_INFO_T	clientInfo;
		  int						sock;
		  void						*ptr;
	  }LIVE_CLIENT_INFO_T;
#define	MAX_CLIENT_NUM		(MAX_DEFAULT_MULTI_THREAD_NUM * 4)	//最大客户端数 = 最大通道数*4

  LIVE_CLIENT_INFO_T		*pRtspClientInfoList;
  OSMutex					clientListMutex;

  RTSP_CLIENT_INFO_T	*pRtspClientList;
  int	GetRTSPClientList(RTSP_CLIENT_INFO_T	**pClientInfo, int *clientNum);
#endif
};
