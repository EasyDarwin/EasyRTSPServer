...
TCPStreamSink::~TCPStreamSink() {
  // Turn off any pending background handling of our output socket:
	...
	_TRACE(TRACE_LOG_DEBUG, (char*)"TCPStreamSink::~TCPStreamSink() 从[%s]中移除该Socket[%d].\n", envir().GetEnvirName(), fOutputSocketNum);
...
void TCPStreamSink::socketWritableHandler1() {
	...
	_TRACE(TRACE_LOG_DEBUG, (char*)"TCPStreamSink::socketWritableHandler1() 从[%s]中移除该Socket[%d].\n", envir().GetEnvirName(), fOutputSocketNum);
