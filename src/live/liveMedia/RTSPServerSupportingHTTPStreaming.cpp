...
RTSPServerSupportingHTTPStreaming::createNew(UsageEnvironment& env, Port rtspPort,
  ...
  int ourSocket = setUpOurSocket4(env, rtspPort);
...
::RTSPServerSupportingHTTPStreaming(UsageEnvironment& env, int ourSocket, Port rtspPort,
				    UserAuthenticationDatabase* authDatabase, unsigned reclamationTestSeconds)
  : RTSPServer(env, ourSocket, ourSocket, rtspPort, authDatabase, reclamationTestSeconds, NULL, NULL) {
}
...
RTSPServerSupportingHTTPStreaming::createNewClientConnection(int clientSocket, LIVE_NET_ADDRESS_SOCKADDR_IN clientAddr, LIVE_IP_VER_ENUM ipVer) {
  return new RTSPClientConnectionSupportingHTTPStreaming(*this, clientSocket, clientAddr, ipVer);
}

RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming
::RTSPClientConnectionSupportingHTTPStreaming(RTSPServer& ourServer, int clientSocket, LIVE_NET_ADDRESS_SOCKADDR_IN clientAddr, LIVE_IP_VER_ENUM ipVer)
  : RTSPClientConnection(ourServer, clientSocket, clientAddr, ipVer),
...
::handleHTTPCmd_StreamingGET(char const* urlSuffix, char const* fullRequestStr) {
  ...
  do {
    ...
    do {
      ServerMediaSession* session = fOurServer.lookupServerMediaSession(&envir(), 0, NULL, streamName, True);
      if (session == NULL) {
	...
	break;
      ...
      subsession->getStreamParameters(NULL, fClientSessionId, tmpInAddr, clientRTPPort,clientRTCPPort, -1,0,0, destinationAddress,destinationTTL, isMulticast, serverRTPPort,serverRTCPPort, streamToken, 0, LIVE_IP_VER_4, (char *)fullRequestStr);
  ...
  ServerMediaSession* session = fOurServer.lookupServerMediaSession(&envir(), 0, NULL, urlSuffix, True);
  if (session == NULL) {
    handleHTTPCmd_notFound();
    return;
  }
