...
MediaSession::MediaSession(UsageEnvironment& env)
  : Medium(env),
    ...
    fMediaSessionType(NULL), fSessionName(NULL), fSessionDescription(NULL), fControlPath(NULL) {
		fSourceFilterAddr.sin_addr.s_addr = 0;
...
static Boolean parseSourceFilterAttribute(char const* sdpLine,
  ...
  do {
    ...
    NetAddressList addresses(sourceName, LIVE_IP_VER_4);		//gavin 2018.03.26
    if (addresses.numAddresses() == 0) break;

    LIVE_NET_ADDRESS_INADDR sourceAddrBits
      = *(LIVE_NET_ADDRESS_INADDR*)(addresses.firstAddress()->data());

	LIVE_NET_ADDRESS_INADDR	tmpInAddr;
	memset(&tmpInAddr, 0x00, sizeof(LIVE_NET_ADDRESS_INADDR));

    if (0 == memcmp(&tmpInAddr, &sourceAddrBits, sizeof(LIVE_NET_ADDRESS_INADDR))) break;

	memcpy(&sourceAddr,  &sourceAddrBits, sizeof(LIVE_NET_ADDRESS_INADDR));
	//sourceAddr.sin_addr.s_addr = sourceAddrBits;
    result = True;
  } while (0);
...
::initiateByMediaType(char const* mimeType,
  ...
  while ((subsession = iter.next()) != NULL) {
    ...
    if (!wasAlreadyInitiated) {
      ...
      if (!subsession->initiate(LIVE_IP_VER_4, useSpecialRTPoffset)) return False;		//gavin ?Y?∩那1車?,?豕???aipv4
    }
...
Boolean MediaSubsession::initiate(LIVE_IP_VER_ENUM ipVer, int useSpecialRTPoffset) {
  ...
  do {
    if (fCodecName == NULL) {
      ...
      break;
		...
		LIVE_NET_ADDRESS_INADDR tempAddr = connectionEndpointAddress();
		//tempAddr.sin_addr.s_addr = connectionEndpointAddress();
		//memcpy(&tempAddr, &connectionEndpointAddress(), sizeof(LIVE_NET_ADDRESS_INADDR));
        // This could get changed later, as a result of a RTSP "SETUP"

		if (fClientPortNum != 0 && (honorSDPPortChoice || IsMulticastAddress(tempAddr, ipVer))) {
			...
			if (isSSM(ipVer)) {
				fRTPSocket = new Groupsock(env(), tempAddr, fSourceFilterAddr, fClientPortNum, 0, ipVer);
			} else {
				fRTPSocket = new Groupsock(env(), tempAddr, fClientPortNum, 255, 0, ipVer);
			}
	...
	if (fMultiplexRTCPWithRTP) {
	  ...
	  if (isSSM(ipVer)) {
	    fRTCPSocket = new Groupsock(env(), tempAddr, fSourceFilterAddr, rtcpPortNum, 0, ipVer);
	  } else {
	    fRTCPSocket = new Groupsock(env(), tempAddr, rtcpPortNum, 255, 0, ipVer);
	  }
	}
      }
    } else {
	...
	// Create a new socket:
	if (isSSM(ipVer)) {
	  fRTPSocket = new Groupsock(env(), tempAddr, fSourceFilterAddr, 0, 0, ipVer);
	} else {
	  fRTPSocket = new Groupsock(env(), tempAddr, 0, 255, 0, ipVer);
	}
	if (fRTPSocket == NULL) {
	...
	Port clientPort(0);
	if (!getSourcePort(env(), fRTPSocket->socketNum(), clientPort, ipVer)) {
	  break;
	}
    ...
    if (isSSM(ipVer) && fRTCPSocket != NULL) {
...
LIVE_NET_ADDRESS_INADDR MediaSubsession::connectionEndpointAddress() const {
  do {
    ...
    // Now, convert this name to an address, if we can:
    NetAddressList addresses(endpointString, fRTPSocket->socketIPVer());
    if (addresses.numAddresses() == 0) break;

    return *(LIVE_NET_ADDRESS_INADDR*)(addresses.firstAddress()->data());
  } while (0);

  // No address known:

  static LIVE_NET_ADDRESS_INADDR fix_netAddress;
  memset(&fix_netAddress, 0x00, sizeof(LIVE_NET_ADDRESS_INADDR));

  return fix_netAddress;
}

void MediaSubsession::setDestinations(LIVE_NET_ADDRESS_INADDR defaultDestAddress) {
  ...
  LIVE_NET_ADDRESS_INADDR destAddress = connectionEndpointAddress();

	LIVE_NET_ADDRESS_INADDR tmpInAddr;
	memset(&tmpInAddr, 0x00, sizeof(LIVE_NET_ADDRESS_INADDR));
	if (0 == memcmp(&tmpInAddr, &destAddress, sizeof(LIVE_NET_ADDRESS_INADDR)))
	{
		memcpy(&destAddress, &defaultDestAddress, sizeof(LIVE_NET_ADDRESS_INADDR));
	}
  //if (destAddress == 0) destAddress = defaultDestAddress;
  LIVE_NET_ADDRESS_INADDR destAddr; 
  memcpy(&destAddr, &destAddress, sizeof(LIVE_NET_ADDRESS_INADDR));
  //destAddr.sin_addr.s_addr = destAddress;
  ...
  if (fRTCPSocket != NULL && !isSSM(LIVE_IP_VER_4) && !fMultiplexRTCPWithRTP) {			//gavin 2018.03.26 ∩?∩|米?ipVer?a?赤?∫??2?米¯車?, 1那∩?∩|足??aIPV4,2?車～?足
...
Boolean MediaSubsession::parseSDPAttribute_fmtp(char const* sdpLine) {
  ...
  do {
    ...
    unsigned const sdpLineLen = (unsigned)strlen(sdpLine);
