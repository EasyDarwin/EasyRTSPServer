...
::MPEG2TransportUDPServerMediaSubsession(UsageEnvironment& env,
                                         char const* inputAddressStr, Port const& inputPort, Boolean inputStreamIsRawUDP)
  : OnDemandServerMediaSubsession(env, True/*reuseFirstSource*/, "MPEG2TransportUDPServerMediaSubsession", "MediaType"),
...
::createNewStreamSource(unsigned/* clientSessionId*/, unsigned& estBitrate) {
  ...
  if (fInputGroupsock == NULL) {
    ...
    LIVE_NET_ADDRESS_INADDR inputAddress;
	memset(&inputAddress, 0x00, sizeof(LIVE_NET_ADDRESS_INADDR));
	if (NULL != fInputAddressStr)
	{
		LIVE_NET_ADDRESS_INADDR inaddr = our_inet_addr(fInputAddressStr, LIVE_IP_VER_4);
		memcpy(&inputAddress, &inaddr, sizeof(LIVE_NET_ADDRESS_INADDR));
	}
	//inputAddress.sin_addr.s_addr = fInputAddressStr == NULL ? 0 : our_inet_addr(fInputAddressStr);
    fInputGroupsock = new Groupsock(envir(), inputAddress, fInputPort, 255, 0, LIVE_IP_VER_4);		//gavin 2018.03.26
  }
