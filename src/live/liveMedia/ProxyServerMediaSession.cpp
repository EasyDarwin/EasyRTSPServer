...
private: // redefined virtual functions
  ...
  virtual Groupsock* createGroupsock(LIVE_NET_ADDRESS_INADDR const& addr, Port port, LIVE_IP_VER_ENUM ipVer);
...
Groupsock* ProxyServerMediaSession::createGroupsock(LIVE_NET_ADDRESS_INADDR const& addr, Port port, int clientSock, LIVE_IP_VER_ENUM ipVer) {
...
void ProxyServerMediaSession::resetDESCRIBEState() {
  ...
  if (fOurMediaServer != NULL) {
    ...
    fOurMediaServer->closeAllClientSessionsForServerMediaSession(this, True);
  }
...
::ProxyServerMediaSubsession(MediaSubsession& mediaSubsession,
  ...
  : OnDemandServerMediaSubsession(mediaSubsession.parentSession().envir(), True/*reuseFirstSource*/,
				 "ProxyServerMediaSubsession", "mediaType", initialPortNum, multiplexRTCPWithRTP),
...
FramedSource* ProxyServerMediaSubsession::createNewStreamSource(unsigned clientSessionId, unsigned& estBitrate) {
  ...
  if (fClientMediaSubsession.readSource() == NULL) {
    ...
    fClientMediaSubsession.initiate(LIVE_IP_VER_4);		//gavin 2018.03.26
    if (verbosityLevel() > 0) {
...
Groupsock* ProxyServerMediaSubsession::createGroupsock(LIVE_NET_ADDRESS_INADDR const& addr, Port port, LIVE_IP_VER_ENUM ipVer) {
  ProxyServerMediaSession* parentSession = (ProxyServerMediaSession*)fParentSession;
  return parentSession->createGroupsock(addr, port, 0, ipVer);
}
