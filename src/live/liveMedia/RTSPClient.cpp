...
Boolean RTSPClient::parseRTSPURL(UsageEnvironment& env, char const* url,
  ...
  do {
	...
	*to = '\0';
    ...
    NetAddressList addresses(parseBuffer, LIVE_IP_VER_4);	//gavin 2018.03.26
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
  ...
  : Medium(env),
	...
	memset(&fServerAddress, 0x00, sizeof(fServerAddress));
  setBaseURL(rtspURL);
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0) {
    ...
    if (streamUsingTCP) { // streaming over the RTSP connection
      ...
      rtcpNumber = fTCPStreamIdCount++;
		...
		LIVE_NET_ADDRESS_INADDR	connectionAddress;
		LIVE_NET_ADDRESS_INADDR tmpConnAddr = subsession.connectionEndpointAddress();
		memcpy(&connectionAddress, &tmpConnAddr, sizeof(LIVE_NET_ADDRESS_INADDR));

		int addrInvalid = 0x00;
		LIVE_NET_ADDRESS_INADDR tmpInAddr;
		memset(&tmpInAddr, 0x00, sizeof(LIVE_NET_ADDRESS_INADDR));
		if (live_ip_ver == LIVE_IP_VER_4)
		{
			if (connectionAddress.sin_addr.s_addr == 0)	addrInvalid = 0x01;
		}
		else
		{
			if (0 == memcmp(&connectionAddress.sin6_addr, &tmpInAddr.sin6_addr, sizeof(tmpInAddr.sin6_addr)))
			{
				addrInvalid = 0x01;
			}
		}
	...
	= IsMulticastAddress(connectionAddress, live_ip_ver) || (addrInvalid == 1 && forceMulticastOnUnspecified);
    ...
    delete[] username;
    delete[] password;
    LIVE_NET_ADDRESS_INADDR serverAddress = *(LIVE_NET_ADDRESS_INADDR*)(destAddress.data());
    AddressString serverAddressString(serverAddress, live_ip_ver);		//gavin 2018.03.26
	...
	unsigned counter;
      ...
      gettimeofdayEx(&seedData.timestamp, NULL);
...
void RTSPClient::resetTCPSockets() {
  if (fInputSocketNum >= 0) {
	...
	_TRACE(TRACE_LOG_DEBUG, (char*)"RTSPClient::resetTCPSockets() 从[%s]中移除该Socket1[%d].\n", envir().GetEnvirName(), fInputSocketNum);
    ...
    if (fOutputSocketNum != fInputSocketNum) {
		...
		_TRACE(TRACE_LOG_DEBUG, (char*)"RTSPClient::resetTCPSockets() 从[%s]中移除该Socket2[%d].\n", envir().GetEnvirName(), fOutputSocketNum);
...
int RTSPClient::openConnection() {
  do {
    ...
    fInputSocketNum = setupStreamSocket4(envir(), 0);
    ...
    fServerAddress = *(LIVE_NET_ADDRESS_INADDR*)(destAddress.data());
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
	...
	if (live_ip_ver == LIVE_IP_VER_4)		//IPV4
	{

		MAKE_SOCKADDR_IN(remoteName, fServerAddress.sin_addr.s_addr, htons(remotePortNum));
	  if (fVerbosityLevel >= 1) {
		  LIVE_NET_ADDRESS_SOCKADDR_IN	tmpAddr;
		  memset(&tmpAddr, 0x00, sizeof(LIVE_NET_ADDRESS_SOCKADDR_IN));
		  memcpy(&tmpAddr.saddr4, &remoteName, sizeof(remoteName));
		envir() << "Opening connection to " << AddressString(tmpAddr, LIVE_IP_VER_4).val() << ", port " << remotePortNum << "...\n";
	  }
	...
	else
	{
		...
		MAKE_SOCKADDR_IN6(remoteName, fServerAddress.sin6_addr, htons(remotePortNum));
	  if (fVerbosityLevel >= 1) {
		  LIVE_NET_ADDRESS_SOCKADDR_IN	tmpAddr;
		  memset(&tmpAddr, 0x00, sizeof(LIVE_NET_ADDRESS_SOCKADDR_IN));
		  memcpy(&tmpAddr.saddr6, &remoteName, sizeof(remoteName));
		envir() << "Opening connection to " << AddressString(tmpAddr, LIVE_IP_VER_6).val() << ", port " << remotePortNum << "...\n";
	  }
	  if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0) {
		int const err = envir().getErrno();
		if (err == EINPROGRESS || err == EWOULDBLOCK) {
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession& subsession, char const* sessionParamsStr, char const* transportParamsStr,
  ...
  do {
	...
	increaseReceiveBufferTo(envir(), fInputSocketNum, 50*1024);
      ...
      LIVE_NET_ADDRESS_INADDR destAddress = subsession.connectionEndpointAddress();

	  LIVE_NET_ADDRESS_INADDR	tmpInAddr;
	  if (0 == memcmp(&tmpInAddr, &destAddress, sizeof(destAddress)))
	  {
		  memcpy(&destAddress, &fServerAddress, sizeof(fServerAddress));
	  }
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode, char* responseString) {
  ...
  do {
    ...
    fOutputSocketNum = setupStreamSocket4(envir(), 0);
...
void RTSPClient::connectionHandler1() {
  // Restore normal handling on our sockets:
	...
	_TRACE(TRACE_LOG_DEBUG, (char*)"RTSPClient::connectionHandler1() 从[%s]中移除该Socket[%d].\n", envir().GetEnvirName(), fOutputSocketNum);
...
void RTSPClient::incomingDataHandler1() {
  LIVE_NET_ADDRESS_SOCKADDR_IN dummy; // 'from' address - not used
...
::createNew(UsageEnvironment& env, onRTSPClientCreationFunc* creationFunc, Port ourPort,
  ...
  int ourSocket = setUpOurSocket4(env, ourPort);
...
::HandlerServerForREGISTERCommand(UsageEnvironment& env, onRTSPClientCreationFunc* creationFunc, int ourSocket, Port ourPort,
                                  UserAuthenticationDatabase* authDatabase, int verbosityLevel, char const* applicationName)
  : RTSPServer(env, ourSocket, ourSocket, ourPort, authDatabase, 30/*small reclamationTestSeconds*/, NULL, NULL),
