...
Boolean parseRTSPRequestString(char const* reqStr,
  ...
  unsigned i = 0;
	...
	//增加RTCP包的判断
	if ( (unsigned char)reqStr[1] == 0xC9 ||		//RTCP_RR
		 (unsigned char)reqStr[1] == 0xCA ||		//RTCP_SDES
		 (unsigned char)reqStr[1] == 0xCB)		//RTCP_BYE
	{
		//if (NULL != strstr(reqStr, "GET_PARAMETER"))
		{
			for (i=0; i<reqStrSize; i++)
			{
				if ( ((unsigned char)reqStr[i] == 'G') &&
					 ((unsigned char)reqStr[i+1] == 'E') &&
					 ((unsigned char)reqStr[i+2] == 'T') &&
					 ((unsigned char)reqStr[i+3] == '_') )
				{

					_TRACE(TRACE_LOG_DEBUG, (char *)"******  此处粘包处理: %s  \n", reqStr);
...
/*
		for (i=0; i<reqStrSize; i++)
		{
			if ((unsigned char)reqStr[i+1] == 0xC9 ||
				(unsigned char)reqStr[i+1] == 0xCA)
			{
				unsigned short usLen = (((unsigned char)reqStr[2] & 0xFF) << 8) | ((unsigned char)reqStr[3]&0xFF);
				if (usLen > 0)
				{
					i+= 4 * usLen;
					i+= 4;
...
*/
  ...
  for (unsigned k = i+1; (int)k < (int)(reqStrSize-5); ++k) {
	...
	reqStr[k+2] == 'S' && reqStr[k+3] == 'P' && reqStr[k+4] == '/') {
	  ...
	  {
		  int len = (int)strlen(resultURLSuffix);
		  if (len > 0)
		  {
			  if (resultURLSuffix[len-1] == '\r')		resultURLSuffix[len-1]='\0';
			  if (resultURLSuffix[len-1] == '\n')	resultURLSuffix[len-1]='\0';

			  len = (int)strlen(resultURLSuffix);
			  for (int i=len-2; len>=2 && i<len; i++)
			  {
				  if (i < 0)	break;
				if (resultURLSuffix[i] == '\r')	resultURLSuffix[i]='\0';
				if (resultURLSuffix[i] == '\n')	resultURLSuffix[i]='\0';
			  }
		  }
	  }
...
Boolean RTSPOptionIsSupported(char const* commandName, char const* optionsResponseString) {
  do {
    ...
    unsigned const commandNameLen = (unsigned)strlen(commandName);
