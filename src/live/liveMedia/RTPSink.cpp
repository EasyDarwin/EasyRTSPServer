...
char* RTPSink::rtpmapLine() const {
  if (rtpPayloadType() >= 96) { // the payload format type is dynamic
    ...
    unsigned rtpmapFmtSize = (unsigned)strlen(rtpmapFmt)
      + 3 /* max char len */ + (unsigned)strlen(rtpPayloadFormatName())
      + 20 /* max int len */ + (unsigned)strlen(encodingParamsPart);
...
::noteIncomingRR(u_int32_t SSRC, LIVE_NET_ADDRESS_SOCKADDR_IN const& lastFromAddress,
...
RTPTransmissionStats::RTPTransmissionStats(RTPSink& rtpSink, u_int32_t SSRC)
  ...
  gettimeofdayEx(&fTimeCreated, NULL);
...
::noteIncomingRR(LIVE_NET_ADDRESS_SOCKADDR_IN const& lastFromAddress,
		 unsigned lossStats, unsigned lastPacketNumReceived,
  ...
  gettimeofdayEx(&fTimeReceived, NULL);
