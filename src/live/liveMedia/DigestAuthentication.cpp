...
char const* Authenticator::computeDigestResponse(char const* cmd,
  ...
  if (fPasswordIsMD5) {
    ...
    unsigned const ha1DataLen = (unsigned)strlen(username()) + 1
      + (unsigned)strlen(realm()) + 1 + (unsigned)strlen(password());
  ...
  unsigned const ha2DataLen = (unsigned)strlen(cmd) + 1 + (unsigned)strlen(url);
  ...
  unsigned const digestDataLen
    = 32 + 1 + (unsigned)strlen(nonce()) + 1 + 32;
