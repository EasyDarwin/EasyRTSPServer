...
void RTCPInstance::noteArrivingRR(LIVE_NET_ADDRESS_SOCKADDR_IN const& fromAddressAndPort,
				  int tcpSocketNum, unsigned char tcpStreamChannelId) {
  // If a 'RR handler' was set, call it now:

  // Specific RR handler:
  if (fSpecificRRHandlerTable != NULL) {
    LIVE_NET_ADDRESS_INADDR fromAddr;

    portNumBits fromPortNum;
    if (tcpSocketNum < 0) {
      // Normal case: We read the RTCP packet over UDP
		//fromAddr = fromAddressAndPort.saddr4.sin_addr.s_addr;
		memcpy(&fromAddr.sin_addr, &fromAddressAndPort.saddr4.sin_addr, sizeof(fromAddr.sin_addr));
		memcpy(&fromAddr.sin6_addr, &fromAddressAndPort.saddr6.sin6_addr, sizeof(fromAddr.sin6_addr));

		if (fRTCPInterface.gs()->socketIPVer() == LIVE_IP_VER_4)
		{
				fromPortNum = ntohs(fromAddressAndPort.saddr4.sin_port);
		}
		else
		{
			fromPortNum = ntohs(fromAddressAndPort.saddr6.sin6_port);
		}
    } else {
      ...
      // Hack: Use the TCP socket and channel id to look up the handler
		if (fRTCPInterface.gs()->socketIPVer() == LIVE_IP_VER_4)
		{
			fromAddr.sin_addr.s_addr = tcpSocketNum;
		}
		else
		{
			memcpy(&fromAddr.sin6_addr.s6_addr, &tcpSocketNum, 4);
		}
	...
	inet_pton(AF_INET6, buff, &tmpInAddr.sin6_addr.s6_addr);

    Port fromPort(fromPortNum);
    RRHandlerRecord* rrHandler
		= (RRHandlerRecord*)(fSpecificRRHandlerTable->Lookup(fromAddr, tmpInAddr, fromPort, RTCPgs()->socketIPVer()));
    if (rrHandler != NULL) {
...
void RTCPInstance
::setSpecificRRHandler(LIVE_NET_ADDRESS_INADDR fromAddress, Port fromPort,
  ...
  if (fSpecificRRHandlerTable == NULL) {
    ...
    const char *buff = "ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"; 
	inet_pton(AF_INET6, buff, &tmpInAddr.sin6_addr.s6_addr);
	RRHandlerRecord* existingRecord = (RRHandlerRecord*)fSpecificRRHandlerTable->Add(fromAddress, tmpInAddr, fromPort, rrHandler, RTCPgs()->socketIPVer());
  delete existingRecord; // if any

}

void RTCPInstance
::unsetSpecificRRHandler(LIVE_NET_ADDRESS_INADDR fromAddress, Port fromPort) {
  ...
  RRHandlerRecord* rrHandler
    = (RRHandlerRecord*)(fSpecificRRHandlerTable->Lookup(fromAddress, tmpInAddr, fromPort, RTCPgs()->socketIPVer()));
  if (rrHandler != NULL) {
    fSpecificRRHandlerTable->Remove(fromAddress, tmpInAddr, fromPort, RTCPgs()->socketIPVer());
    delete rrHandler;
  }
}
...
void RTCPInstance
::injectReport(u_int8_t const* packet, unsigned packetSize, LIVE_NET_ADDRESS_SOCKADDR_IN const& fromAddress) {
...
void RTCPInstance::incomingReportHandler1() {
  do {
	...
	if (RTCPgs()->wasLoopedBackFromUs(envir(), fromAddress, fRTCPInterface.gs()->socketIPVer())) {
...
void RTCPInstance
::processIncomingReport(unsigned packetSize, LIVE_NET_ADDRESS_SOCKADDR_IN const& fromAddressAndPort,
			int tcpSocketNum, unsigned char tcpStreamChannelId) {
  do {
    ...
    if (tcpSocketNum < 0) {
      ...
      fprintf(stderr, "address %s, port %d", AddressString(fromAddressAndPort.saddr4.sin_addr).val(), ntohs(fromAddressAndPort.saddr4.sin_port));
    } else {
	...
	// the port number:
	reportSenderSSRC = fromAddressAndPort.saddr4.sin_addr.s_addr^fromAddressAndPort.saddr4.sin_port;	//gavin 2018.03.21 ??????  ????????????
      }
#endif
...
SDESItem::SDESItem(unsigned char tag, unsigned char const* value) {
  unsigned length = (unsigned)strlen((char const*)value);
