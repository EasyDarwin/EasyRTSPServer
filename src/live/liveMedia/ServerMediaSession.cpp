...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
  ...
  : Medium(env), fIsSSM(isSSM), fSubsessionsHead(NULL),
    ...
    fReferenceCount(0), fDeleteWhenUnreferenced(False),fStreamStatus(0), mFlag(LIVE_FLAG){
  ...
  gettimeofdayEx(&fCreationTime, NULL);
}
...
void ServerMediaSession::testScaleFactor(float& scale) {
  ...
  if (NULL == fSubsessionsHead || fSubsessionsHead->GetFlag() != LIVE_FLAG)
  {
	  _TRACE(TRACE_LOG_DEBUG, (char *)"#############[%s] fSubsessionsHead->GetFlag() != LIVE_FLAG\n", envir().GetEnvirName());
	  return;
  }
...
char* ServerMediaSession::generateSDPDescription(int clientSock, LIVE_IP_VER_ENUM ipVer) {
  AddressString ipAddressStr(ourIPAddress(envir(), ipVer), ipVer);
  unsigned ipAddressStrSize = (unsigned)strlen(ipAddressStr.val());

  // For a SSM sessions, we need a "a=source-filter: incl ..." line also:
  char* sourceFilterLine;
  if (fIsSSM) {
    char const* const sourceFilterFmtV4 =
      "a=source-filter: incl IN IP4 * %s\r\n"
      "a=rtcp-unicast: reflection\r\n";

    char const* const sourceFilterFmtV6 =
      "a=source-filter: incl IN IP6 * %s\r\n"
      "a=rtcp-unicast: reflection\r\n";

    unsigned const sourceFilterFmtSize = (unsigned)(strlen(ipVer==LIVE_IP_VER_4?sourceFilterFmtV4:sourceFilterFmtV6)) + ipAddressStrSize + 1;

    sourceFilterLine = new char[sourceFilterFmtSize];
    sprintf(sourceFilterLine, ipVer==LIVE_IP_VER_4?sourceFilterFmtV4:sourceFilterFmtV6, ipAddressStr.val());
  } else {
  ...
  do {
    ...
    for (subsession = fSubsessionsHead; subsession != NULL;
	 subsession = subsession->fNext) {
      char const* sdpLines = subsession->sdpLines(clientSock, ipVer);
      if (sdpLines == NULL) continue; // the media's not available
      sdpLength += (unsigned)strlen(sdpLines);
    }
    ...
    char const* const sdpPrefixFmt4 =
      "v=0\r\n"
    ...
    char const* const sdpPrefixFmt6 =
      "v=0\r\n"
      "o=- %ld%06ld %d IN IP6 %s\r\n"
      "s=%s\r\n"
      "i=%s\r\n"
      "t=0 0\r\n"
      "a=tool:%s%s\r\n"
    ...
    sdpLength += (unsigned)strlen(ipVer==LIVE_IP_VER_4?sdpPrefixFmt4:sdpPrefixFmt6)
      + 20 + 6 + 20 + ipAddressStrSize
      + (unsigned)strlen(fDescriptionSDPString)
      + (unsigned)strlen(fInfoSDPString)
      + (unsigned)strlen(libNameStr) + (unsigned)strlen(libVersionStr)
      + (unsigned)strlen(sourceFilterLine)
      + (unsigned)strlen(rangeLine)
      + (unsigned)strlen(fDescriptionSDPString)
      + (unsigned)strlen(fInfoSDPString)
      + (unsigned)strlen(fMiscSDPLines);
    sdpLength += 1000; // in case the length of the "subsession->sdpLines()" calls below change
    sdp = new char[sdpLength];
    if (sdp == NULL) break;

    // Generate the SDP prefix (session-level lines):
    snprintf(sdp, sdpLength, ipVer==LIVE_IP_VER_4?sdpPrefixFmt4:sdpPrefixFmt6,
    ...
    for (subsession = fSubsessionsHead; subsession != NULL;
	 subsession = subsession->fNext) {
      unsigned mediaSDPLength = (unsigned)strlen(mediaSDP);
...
ServerMediaSubsession::ServerMediaSubsession(UsageEnvironment& env)
  : Medium(env),
    ...
    fNext(NULL), fTrackNumber(0), fTrackId(NULL), mFlag(LIVE_FLAG) {
		memset(&fServerAddressForSDP, 0x00, sizeof(fServerAddressForSDP));
}
...
void ServerMediaSubsession::deleteStream(UsageEnvironment  *pEnv, unsigned /*clientSessionId*/,
					 void*& /*streamToken*/) {
  // default implementation: do nothing
}
...
void ServerMediaSubsession::setServerAddressAndPortForSDP(LIVE_NET_ADDRESS_INADDR addressBits,
